class Solution {
    public int[][] buildMatrix(int k, int[][] rowConditions, int[][] colConditions) {
        List<Integer> row_sort = topo_sort(rowConditions, k);
        List<Integer> col_sort = topo_sort(colConditions, k);
        if (row_sort.isEmpty() || col_sort.isEmpty())
            return new int[0][0];
        ...
        for (int ind = 0; ind < row_sort.size(); ++ind) {
            value_position.get(row_sort.get(ind))[0] = ind;
        }
        for (int ind = 0; ind < col_sort.size(); ++ind) {
            value_position.get(col_sort.get(ind))[1] = ind;
        }
